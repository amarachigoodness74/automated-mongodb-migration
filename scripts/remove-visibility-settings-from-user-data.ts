...
const removeVisibilitySettingsFromUserData = async () => {
  try {
    ...
    logger.info(
      "Migration to remove visibility settings to user data successfully completed for ALL incubators"
    );
    process.exit(0);
  } catch (e) {
    logger.error(
      "Migration to remove visibility settings to user data was unsuccessful with error: ",
      e
    );
    process.exit(1);
  }
