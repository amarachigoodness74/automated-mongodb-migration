...
const addVisibilitySettingsToUserData = async () => {
  try {
    ...
    logger.info(
      "Migration to add visibility settings to user data successfully completed for ALL users"
    );
    process.exit(0);
  } catch (e) {
    logger.error(
      "Migration to add visibility settings to user data was unsuccessful with error: ",
      e
    );
    process.exit(1);
  }
